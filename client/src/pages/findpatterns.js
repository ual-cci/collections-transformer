...
const FindPatterns = ({ user }) => {
  ...
  const handleAnalyzeClick = async (e) => {
    ...
    const analysisMessage = {
      id: newMessageId,
      type: 'analysis',
      content: "$ Starting test analysis...\n",
      timestamp: new Date()
    };
    ...
    try {
      ...
      const response = await fetch(
        (process.env.NEXT_PUBLIC_SERVER_URL || "") + "/backend/findpatterns_create?",
        requestOptions
      );
      ...
      <div className="container">
        ...
        <div className="patterns-layout">
          <div className="patterns-left">
            <div className="patterns-selection-info">
              <h6>
                {getSelectedAnalyser() && getSelectedDataset() 
                  ? `(Model) ${getSelectedAnalyser().name} → (Dataset) ${getSelectedDataset().name}`
                  : "Select a model and dataset to start analysis"
                }
              </h6>
            ...
            {chatMessages.length > 0 ? (
              ...
              <div style={{
                ...
                <h6 style={{ marginBottom: '0.5rem', color: '#495057' }}>No Analysis Session</h6>

                <p style={{ margin: 0, fontSize: '0.9rem' }}>
                  Add elements from the dataset and click "Analyse Data", or "Add Annotation" to add notes.
                </p>
              </div>
            )}
            ...
            <div style={{ 
              ...
              <div style={{ 
                ...
                <button 
                  ...
                  style={{
                    backgroundColor: (isAnalyzing || selectedItems.size === 0) ? '#6c757d' : 'black',
                    color: 'white',
                    border: 'none',
                    padding: '0.5rem 1rem',
                    borderRadius: '4px',
                  ...
                  onMouseEnter={(e) => {
                    if (!isAnalyzing && selectedItems.size > 0) e.target.style.backgroundColor = '#333';
                  }}
                  onMouseLeave={(e) => {
                    if (!isAnalyzing && selectedItems.size > 0) e.target.style.backgroundColor = 'black';
                  }}
                >
                ...
                <button 
                  ...
                  style={{
                    backgroundColor: chatMessages.length === 0 ? '#6c757d' : 'black',
                    color: 'white',
                    border: 'none',
                    padding: '0.5rem 1rem',
                    borderRadius: '4px',
                  ...
                  onMouseEnter={(e) => {
                    if (chatMessages.length > 0) e.target.style.backgroundColor = '#333';
                  }}
                  onMouseLeave={(e) => {
                    if (chatMessages.length > 0) e.target.style.backgroundColor = 'black';
                  }}
                >
                  Save Analysis
                </button>
          ...
          <div className="patterns-right">
            <div className="patterns-models">
              <h6>Models</h6>

              <RadioList
                items={analysers}
                name="analyser"
                selectedId={selectedAnalyser}
                onChange={handleAnalyserChange}
                getLabel={item => item.name}
              />
            </div>
            <div className="patterns-datasets">
              <h6>Datasets</h6>
...
export default withPageAuthRequired(FindPatterns);
